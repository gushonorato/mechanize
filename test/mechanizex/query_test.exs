defmodule Mechanizex.QueryTest do
  ...
  describe ".match_criteria?" do
    ...
    test "by text with regex" do
      assert @subject
             |> Enum.filter(&Query.match_criteria?(&1, text: ~r/Googl/))
             |> Enum.map(&Element.text/1) == ["Google"]
    end
  end
end
